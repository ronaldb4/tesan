...
from src.concept_embedding.data.__dataset_template__ import DatasetTemplate
from src.utils.file import save_file,load_file


class ConceptSkipDataset(DatasetTemplate):
    def __init__(self):
        super(ConceptSkipDataset, self).__init__()
        self.contexts = None
        self.intervals = None
        self.labels = None
    ...
    def process_data(self):
        ...
        save_file({'contexts': contexts, 'intervals': intervals, 'labels': labels}, self.processed_path)
        return contexts, intervals, labels

    def load_data(self):
        is_load, data = load_file(self.processed_path, 'processed data', 'pickle')
        if not is_load:
